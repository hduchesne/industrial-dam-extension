...
<c:set var="imageNode" value="${currentNode.properties['avatar'].node}"/>
<template:addCacheDependency node="${imageNode}"/>

<c:choose>
    <c:when test="${!empty imageNode && jcr:isNodeType(imageNode, 'cloudymix:cloudyAsset')}">
        <template:module node="${imageNode}" view="hidden.getURL" var="avatarURL" editable="false" templateType="txt">
    ...
    <c:otherwise>
        <c:url var="avatarURL" value="${imageNode.url}" context="/"/>
    </c:otherwise>
</c:choose>
